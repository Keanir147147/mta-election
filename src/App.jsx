...
function runSTV(ballots, candidates = CANDIDATES) {
 ...
 const minV = Math.min(...active.map(ci => counts[ci]))
      ...
      // Tie-break order: first try look-back at earlier rounds (find a round
      // where the tied candidates had different vote counts). If still tied
      // all the way back, use Condorcet head-to-head from the original
      // ballots. As a last resort (perfect symmetry), falls back to last
      // candidate in the tied list.
      let toElim = tied[tied.length - 1]
      if (tied.length > 1) {
        let resolvedByLookback = false
